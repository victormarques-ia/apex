...
// Action to get the athlete list
export async function getAthletesList(formData: FormData) {
    return actionHandlerWithValidation(
      ...
      async (data) => {
        ...
        // Fetch the athletes
        // change to agency athletes api
        const result = await fetchFromApi(`/api/agencies/my-athletes?${queryParams.toString()}`, {
          method: 'GET',
        });
        
        if (!result.data?.data) {
          throw new Error(result.error?.messages[0] || 'Erro ao buscar treinadores');
        }
      ...
      },
      {
        ...
        onFailure: (error) => {
          return {
            success: false,
            error,
            message: 'Falha ao buscar treinadores',
          };
        },
      }
    );
