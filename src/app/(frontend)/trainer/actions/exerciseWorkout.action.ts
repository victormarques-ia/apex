...
export async function getAllExercisesWorkoutsAction(_state: unknown, formData: FormData) {
  return actionHandlerWithValidation(
    formData,
    async (data) => {
      console.log('Getting all exercises workouts')

      // Fetch all exercises with no filters
      const workoutPlanId = data.workoutPlanId
      const queryParams = new URLSearchParams()
      queryParams.append('workoutPlanId', workoutPlanId)
