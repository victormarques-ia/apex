...
// Action to get all workout plans for a specific athlete
export async function getAthleteWorkoutPlansAction(_state: unknown, formData: FormData) {
  return actionHandlerWithValidation(
    ...
    async (data) => {
      ...
      const result = await fetchFromApi(
        `/api/trainers/athlete-workout-plans?${queryParams.toString()}`,
        {
          method: 'GET',
        },
      )
      if (!result.data) {
        console.error('Workout plans fetch API error:', result.error)
        throw new Error(result.error?.messages[0] || 'Erro ao buscar planos de treino')
      }
    ...
    },
    {
      ...
      onFailure: (error) => {
        console.error('Workout plans fetch failure:', error)
        return {
          success: false,
          error,
          message: 'Falha ao buscar planos de treino',
        }
      },
    },
  )
